...
function fillOrderArray()  
{
    ...
    if (var_order_list.length == undefined) {       // 1 seul résultat à la recherche
        ...
        // console.log('fillOrderArray 1: site_code = ', site_code);
        // console.log('var_order_list = ', var_order_list);

        // var_order_list.toEvaluate = (toEvaluate) ? true : false;
        var notation = (isEmpty(var_order_list.u_customer_satisfaction)) ? "" : setNotation(var_order_list.u_customer_satisfaction, 0);

        // .tz(var_order_list.tasksla_timezone)
        if (var_order_list.opened_at != "") {
            opened_at = moment(var_order_list.opened_at).format("DD/MM/YYYY HH:mm:ss");
        }
        if (var_order_list.expected_start != "") {
            expected_start = moment(var_order_list.expected_start).format("DD/MM/YYYY HH:mm:ss");
        }
        if (var_order_list.closed_at != "") {
            closed_at = moment(var_order_list.closed_at).format("DD/MM/YYYY HH:mm:ss");
        }getWorkOrderListFromServiceNow
        // RMPApplication.debug("fillOrderArray : number = " + var_order_list.number + "\n opened_at = " + opened_at + "\n closed_at = " + var_order_list.closed_at);
        // var site_name = (var_order_list.site.length == 0) ? var_order_list.location : var_order_list.site[0].location_name;
        var site_name = (!isEmpty(var_order_list.location)) ? var_order_list.location : "";
        var row = new Array (
            "",
            // "<span>&nbsp;<a  href=\"#\" onClick=\"displayDetail(0);\" target=\"_self\"><i class=\"fa fa-search fa-lg loupe\" aria-hidden=\"true\"></i></a></span>",
            "<button onClick=\"displayDetail(0);\" class=\"btn_no_style\">&nbsp;<i class=\"fa fa-search fa-lg loupe\" aria-hidden=\"true\"></i></button>",
            var_order_list.number,
            var_order_list.short_description.substring(0,45),
            // "<div id='id_state0'>" + getStatusLabel(var_order_list.state) + "</div>",
        ...
        for (i=0; i < var_order_list.length; i++) {
            try {
                ...
                var site_code = $.trim(buf[buf.length - 1]);
                // console.log('fillOrderArray 2: site_code = ', site_code);
                // console.log('var_order_list[i] = ', var_order_list[i]);

                // var_order_list[i].toEvaluate = (toEvaluate) ? true : false;
                var notation = (isEmpty(var_order_list[i].u_customer_satisfaction)) ? "" : setNotation(var_order_list[i].u_customer_satisfaction, i);

                // .tz(var_order_list[i].tasksla_timezone)
                if (var_order_list[i].opened_at != "") {
                    opened_at = moment(var_order_list[i].opened_at).format("DD/MM/YYYY HH:mm:ss");
                }
                if (var_order_list[i].expected_start != "") {
                    expected_start = moment(var_order_list[i].expected_start).format("DD/MM/YYYY HH:mm:ss");
                }
                if (var_order_list[i].closed_at != "") {
                    closed_at = moment(var_order_list[i].closed_at).format("DD/MM/YYYY HH:mm:ss");
                }
                // RMPApplication.debug("fillOrderArray : number = " + var_order_list[i].number + "\n opened_at = " + opened_at + "\n closed_at = " + var_order_list[i].closed_at);
                // var site_name = (var_order_list[i].site.length == 0) ? var_order_list[i].location : var_order_list[i].site[0].location_name;
                var site_name = (!isEmpty(var_order_list[i].location)) ? var_order_list[i].location : "";
                var row = new Array (
                    "",
                    // "<span>&nbsp;<a  href=\"#\" onClick=\"displayDetail(" + i + ");\" target=\"_self\"><i class=\"fa fa-search fa-lg loupe\" aria-hidden=\"true\"></i></a></span>",
                    ...
                    // "<div id='id_state" + i + "'>" + getStatusLabel(var_order_list[i].state) + "</div>",
...
function load_img_model() 
{
    ...
    if ( (!isEmpty(selectedCat)) && (!isEmpty(selectedBrand)) && (!isEmpty(selectedModel)) ) {
        ...
        item_model_img += '<div id="id_item_model_img">'
                        + '<img id="id_product_img" src="https://live.runmyprocess.com/live/112501480325272109/upload/5d675a60-25cc-11e7-869b-02b3a23437c9/no_image_available.png" height="200" width="200" alt="Image associée au produit" class="img-thumbnail"><br>'
                        + '</div>';
