...
<!-- Codabar barcode as an SVG image.                          -->
...
<!-- ========================================================= -->
  ...
  <xsl:template name="draw-barcode-codabar">
      ...
      <xsl:variable name="length">
        <xsl:value-of select="string-length($sequence) div 7"/>
      </xsl:variable>

      <!-- Calculate codesequence length -->
      <xsl:variable name="code-width-real" select="$wide-real*(2*($length - 2)+6) + $narrow-real*(5*($length - 2) + 8 + $length - 1)"/>
      ...
      <svg:svg width="{concat($area-width-real div 360, 'mm')}"
      	...
      	<desc xmlns:mydoc="http://example.org/mydoc">
      		<barcode value="{$value}" type="codabar"></barcode>
      	</desc>
      	
        <!-- Prepare actual path-->
        <xsl:variable name="path">
	      <!-- Call template to reqursively draw all bars -->
          <xsl:call-template name="recursive-draw-codabar">
  ...
  <xsl:template name="recursive-draw-codabar">
      ...
      <xsl:choose>
	    <xsl:when test="$position mod 7 = 0 or (($position mod 7) mod 2) = 1">
      ...
      <xsl:if test="($position mod 7) = 0 and string-length($sequence) &gt; 1">
          <!-- Create appropriate path segment -->  
