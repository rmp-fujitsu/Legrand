...
<xsl:stylesheet xmlns:fo="http://www.w3.org/1999/XSL/Format"
                      ...
                      <xsl:when test="@w:header">
                        <xsl:if test="count(../w:hdr/w:p) > 2">
                          <xsl:if test="@w:top > @w:header">
                            <xsl:variable name="diff" select="@w:top - @w:header"/>
                            <xsl:value-of select="(@w:top + ($diff div 2)) div 20"/><xsl:text>pt</xsl:text>
                            <!-- xsl:value-of select="@w:top div 20"/><xsl:text>pt</xsl:text -->
                          </xsl:if>
                          <xsl:if test="@w:header > @w:top">
                            <xsl:variable name="diff" select="@w:header - @w:top"/>
                            <xsl:value-of select="(@w:header + ($diff div 2)) div 20"/><xsl:text>pt</xsl:text>
                            <!-- xsl:value-of select="@w:header div 20"/><xsl:text>pt</xsl:text -->
                          </xsl:if>
                        </xsl:if>
  ...
  <!-- ====================== -->
      ...
      <xsl:template name="ApplyParaProperties">
        ...
        <!-- VS 2006-06-02 Resolving issue#119. -->
        <!-- pPr style applies to trailing Paragraph Break symbol. This is NOT default style for unstyled run's within paragraph -->
        <!-- Commented next line -->
      ...
      <xsl:template match="w:ind">
        <!-- 03/17/2006: Added param "mode" to specify indentations correctly for your needs: mode='left' affects start-indent/text-indent attributes, mode='right' affects end-indent/text-indent attributes -->
        <!-- If "mode" isn't specified, it doesn't affect indent attributes, they are set as they are. -->
  ...
  <!-- ================= -->
      ...
      <xsl:template match="w:color">
        <!-- When w:color is specified under w:p/w:pPr/w:rPr element,
             it's incorrect to apply color attribute for all runs of the paragraph.
             Thus color attribute applies for a run only when w:color is explicitly specified. -->
  ...
  <!-- ============================== -->
      ...
      <xsl:template match="w:tcBorders">
          <xsl:param name="cell"/>
        <!-- (AM): The logic of handling whole table borders is concentrated here.-->
          <!-- apply default style -->
        <!-- Commented because of double calling. First time in elementProperties.xsl -> template match="w:tc" -->
          <!--<xsl:apply-templates select="$default-table-style/w:tblPr/w:tblBorders">
              <xsl:with-param name="cell" select="$cell"/>
          </xsl:apply-templates>-->
          <!-- apply specified style (by ID) for this table -->
        <!-- Commented because of double calling. First time in elementProperties.xsl -> template match="w:tc" -->
          <!--<xsl:apply-templates select="key('styles', ancestor::w:tbl[1]/w:tblPr/w:tblStyle/@w:val)[@w:type='table']/w:tblPr/w:tblBorders">
              <xsl:with-param name="cell" select="$cell"/>
          </xsl:apply-templates>-->
          <!-- apply concrete style for this table -->
        <!-- Commented because of double calling. First time in elementProperties.xsl -> template match="w:tc" -->
          <!--<xsl:apply-templates select="ancestor::w:tbl[1]/w:tblPr/w:tblBorders">
              <xsl:with-param name="cell" select="$cell"/>
          </xsl:apply-templates>-->
          <!-- and finally, set cell-specific border properties -->
        <xsl:apply-templates select="*">
              <xsl:with-param name="cell" select="$cell"/>
          </xsl:apply-templates>
      </xsl:template>
